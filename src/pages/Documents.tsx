  ...
  const fetchDocuments = useCallback(
    async (which: 'my' | 'shared' = 'my', pageToFetch = 1) => {
      ...
      try {
        const response =
          which === 'shared'
            ? await api.getSharedDocuments({ page: pageToFetch, per_page: pageSize })
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center gap-4">
              ...
              {
                isMobile && (
                  <IconButton onClick={() => setMobileSidebarOpen(true)} aria-label="Open menu">
                    <MenuIcon />
                  </IconButton>
                )
              }

              <h1 className="text-xl font-bold text-text-primary">{view === 'my' ? 'My Documents' : 'Shared with me'}</h1>
              {/* removed top links - links now only in sidebar */}
            </div>
            <div className="flex items-end">
              <div className="relative">
                <IconButton onClick={handleOpenProfile} size="medium" title="Profile" aria-haspopup="true" aria-expanded={isProfileOpen ? 'true' : undefined}>
      ...
      {mobileSidebarOpen && (
        <div className="sm:hidden fixed inset-0 z-50 flex">
          <div className="w-64 bg-card-bg p-3 shadow h-full overflow-auto">
            ...
            <nav>
              <button
                onClick={() => { setView('my'); setMobileSidebarOpen(false); }}
              ...
              <button
                onClick={() => { setView('shared'); setMobileSidebarOpen(false); }}
          ...
          {documents.length === 0 ? (
            ...
            <>
              ...
              <div ref={sentinelRef} className="h-8 flex items-center justify-center mt-6">
                ...
                {!hasMore && (
                  <p className="text-center text-sm text-text-secondary">No more documents</p>
                )}
              </div>
            </>
          )}
